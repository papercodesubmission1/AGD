...
class SpGraphAttentionLayer(nn.Module):
    ...
    def forward(self, input, adj):
        ...
        N = input.size()[0]
        edge2didx = adj.nonzero().t()
        edge1didx = (edge2didx[0] * N + edge2didx[1]).to(torch.long)
        ...
        edge_h = torch.cat((h[edge2didx[0, :], :], h[edge2didx[1, :], :]),
                           dim=1).t()
        # edge: 2*D x E
        ...
        attention = torch.ones(N * N).to(dv) * (-1e15)

        attention[edge1didx] = edge_e
